...
@app.route('/records/albums_description/<bandname>/<album>/',methods=['GET'])
def get_album_description(bandname,album):
    response={bandname:'Not Found!'}
    for item in all_records:
        if item["name"]==bandname:
            albums=[x["title"] for x in item["albums"]]
            for v_album in albums:
                if v_album["album"]==album:
                    response=[desc["description"] for desc in v_album if album==v_album]
